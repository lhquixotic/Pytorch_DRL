...
class Base_Agent(object):
    ...
    def print_rolling_result(self):
        ...
        text = """"\r Episode {0}, Average:{5:.2f},  Score: {3: .2f}, Max score seen: {4: .2f}, Rolling score: {1: .2f}, Max rolling score seen: {2: .2f}"""
        sys.stdout.write(text.format(len(self.game_full_episode_scores), self.rolling_results[-1], self.max_rolling_score_seen,
                                     self.game_full_episode_scores[-1], self.max_episode_score_seen,np.mean(self.game_full_episode_scores)))
        sys.stdout.flush()
